# looking for association between health condition and metabolism rate in control group
# on server filename: health_metabolism_controls_updated.R


# only need to look at p6b23 because that was the only one significant in exposed group
# but left rest in because reusing old code (just didn't record outcomes with other var)
...
#exp is people with slow metab and c is faster metab (reusing original code, so kept notation)
...
# 100 iterations of control groups
...
match_df <- match_df %>% filter(exposure == 0) # only keeping controls
match_df <- merge(match_df, score, by="ffFam")
...
match_df$exposure <- match_df$binary #this line is changed to match_df$binary_alc for alc score
...
bmi_t <- t.test(num_wells_10_bmi_expo$ck6cbmi, num_wells_10_bmi_c$ck6cbmi)
...
write.csv(df_loop, "/home/HPA/Hannah/health_metabolism_controls_all_1.csv")

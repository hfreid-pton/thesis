...
# code file name on server: /home/HPA/Hannah/methylation_1_test.R
...
#EPIC Data
samp <- readRDS("/home/HPA/Hannah/BenCode/datafiles/sampStandardEpicNew.RDS")
prob <- readRDS("/home/HPA/Hannah/BenCode/datafiles/probIDsEpic.RDS")
beta <- as.matrix(readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/EPIC_bmiqnorm.RDS"))
...
# remove SNP cpgs
Zhou_cpg <- readRDS("/home/HPA/HPA_group_datasets/MethylationData/ProbeLists/Zhou/EPIC.hg19.manifest.rds")
...
samp_9 <- samp %>% filter(TypeID == "FC09")
samp_15 <- samp %>% filter(TypeID == "FC15")
...
# read INC files
INC <- readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p20.RDS")
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p21.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p22.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p24.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p25.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p26.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p27.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p28.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p29.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p30.RDS"))
INC <- cbind(INC, readRDS("/home/HPA/Jacobson/Lisa/EPIC_data_April2019/INCs_EPIC/INCs_p31.RDS"))
...
#removing individuals not in epic file
num_wells <- num_wells[num_wells$all_ffid %in% samp$ffID, ]
...
samp_9_filt <- samp_filt %>% filter(TypeID == "FC09")
samp_15_filt <- samp_filt %>% filter(TypeID == "FC15")

beta2 <- beta[,colnames(beta) %in% samp_filt$MethID2]
beta_9 <- beta[,colnames(beta) %in% samp_9_filt$MethID2]
beta_15 <- beta[,colnames(beta) %in% samp_15_filt$MethID2]
...
# compared year 9 to year 15 data in controls and separately in exposed individuals
...
# 
# # creating one matrix with m values for samples and ther INCs at yr 15
...
# exposure_vect_9 <- c()
# for (p in colnames(m_9)){
# samp_filt2 <- samp_filt %>% filter(MethID2 == p)
...
# }
# 
# df_exp_9 <- data.frame(colnames(m_9), exposure_vect_9)


# making dataframe linking methIDs to exposure value (0 or 1) 
# making sure this dataframe's methIDs in order of m value matrix
# exposure_vect_15 <- c()
# for (b in colnames(m_15)){
# samp_filt2 <- samp_filt %>% filter(MethID2 == b)
# ffFam_1 <- as.character(samp_filt2$ffFam)
# match_df_filt <- match_df %>% filter(ffFam == ffFam_1)
# exp <- as.integer(match_df_filt$exposure)
...
# }
# 
# df_exp_15 <- data.frame(colnames(m_15), exposure_vect_15)

# vector that says whether a sample is exposed or not
# fact_exp_9 <- factor(exposure_vect_9, labels=c(0,1))
# fact_exp_15 <- factor(exposure_vect_15, labels=c(0,1))
# 
# #RUVM for age 9
...
# var.b_X1.1 <- c(var.b_X1.1, dmp_15$var_b.X1.1)
...
beta_c <- beta[,colnames(beta) %in% samp_c$MethID2]
...
samp_filt2c <- samp_c %>% filter(MethID2 == i)
year1 <- samp_filt2c$TypeID
...
beta_exp <- beta[,colnames(beta) %in% samp_exp$MethID2]
m_exp <- logit(beta_exp) # converting from beta to M
#making INC file that only includes matched control samples
...
samp_filt2 <- samp_exp %>% filter(MethID2 == i)
year1 <- samp_filt2$TypeID
...
#write.csv(df, "/home/HPA/Hannah/EPIC_meth_2-17_100_15.csv")
#write.csv(df_9, "/home/HPA/Hannah/EPIC_meth_2-17_100_9.csv")


write.csv(dmp_exp, "/home/HPA/Hannah/EPIC_meth_2-17_exp_year915.csv")
write.csv(df_c, "/home/HPA/Hannah/EPIC_meth_2-17_c_year915.csv")

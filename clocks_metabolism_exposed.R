# comparing methylation age clocks in exposed slow metabolizers to exposed fast metabolizers
# server code name: /home/HPA/Hannah/clocks_metabolism_exposed.R
...
surv <- surv[surv$ffFam %in% score_exp,]

merged_score <- merge(score, surv, by="ffFam")

#taking out people who moved and moved schools
...
clocks <- clocks[clocks$ffFam %in% merged_score$ffFam,]
...
df_both <- df_both %>% mutate(dnampai_dif = (age_15_dnampai_res - age_9_dnampai_res))
df_both <- df_both %>% mutate(dnampai_pace = ((age_15_dnampai - age_9_dnampai)/chrage_diff))



# setting up vectors to hold p-values, coefficients, and means
# didn't need this because just running 1x (since 1 exposed group)
# but left it in because adapted code from original methylation clock analysis
p_pack_15res <- c()
coef_pack_15res <- c()
p_pack_9res <- c()
coef_pack_9res <- c()
...
merged_both_nn <- merge(df_both, merged_score, by="ffFam") 

#to reuse code from original methylation clock, I reuse exposure variable
# here exposure ==1 is a slower metabolizer, exposure==0 is a faster metabolizer
merged_both_nn$exposure <- merged_both_nn$binary #this line is changed to merged_both_nn$binary_alc for alc score

# seeing if metabolism rate (called "exposure") is associated with methylation age (for each clock)
...
model_nn_15_dnampai <- glm(age_15_dnampai_res ~ exposure, data=merged_both_nn)
model_nn_9_dnampai <- glm(age_9_dnampai_res ~ exposure, data=merged_both_nn)
model_nn_dif_dnampai <- glm(dnampai_dif ~ exposure, data=merged_both_nn)
model_nn_pace_dnampai <- glm(dnampai_pace ~ exposure, data=merged_both_nn)
...
#didn't save results to csv
# ran once and recorded results in excel file

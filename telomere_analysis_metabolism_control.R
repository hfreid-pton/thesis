# comparing TL in 100 iterations of controls with slow metabolism and controls with fast metabolism
#code name on server: /home/HPA/Hannah/telomere_analysis_metabolism_controls.R

#only needed to analyze TL at age 15 but left rest in because resuing code from original TL analysis
...
# running 100 iterations of comparisons of controls comparing slow controls and fast controls
...
library(MatchIt)
  ...
  #only keeping in people that have tl data available
  ...
  tl2 <- tl[tl$ffFam %in% match_df$ffFam,]
  surv5 <- read.csv("/home/HPA/HPA_group_datasets/ffsurvey/FF_allwaves_2018_FFID.csv")
  surv5 <- surv5 %>% select(ffFam, ck6yagem, ch5agem)
  tl2 <- merge(tl2, surv5, by="ffFam")
  tl2 <- merge(tl2, surv5, by ="ffFam")
...
merged_tl <- tl2
merged_tl$exposure <- merged_tl$binary

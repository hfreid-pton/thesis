#comparing methylation age across a variety of clocks between exposed and unexposed controls
# code name on server: /home/HPA/Hannah/all_epi_clocks_EPIC_updated_healthcond_control.R
...
# creating loop to run 100x (100 iterations of matched controls)
for (r in 1:100){
print(r)
...
control <- match_df$ffFam[match_df$exposure == 0] #only keeping unexposed controls
control <- as.character(control)

#reading in survey data to know who has health cond (p6b19)
surv4 <- read.csv("/home/HPA/HPA_group_datasets/ffsurvey/FF_allwaves_2018_FFID.csv")
df_both <- df_both[df_both$ffFam %in% control,]
...
# with methylation age (for each clock) in control group only
...
mean_dnam_15res_exp <- c(mean_dnam_15res_exp, mean(merged_both_nn_exp$age_15_dnam__res))
...
write.csv(df_results, "/home/HPA/Hannah/all_epi_clocks_3-8-healthcond_controls.csv")
write.csv(df_means, "/home/HPA/Hannah/all_epi_clocks_3-8-healthcond_controls_means.csv")

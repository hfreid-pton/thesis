# telomere analysis of slow metabolizer exposed vs. slow metabolizer controls
#code name on server: /home/HPA/Hannah/telomere_analysis_metabolism_slow.R

library(dplyr)
library(tidyr)
library(stringr)

# making vectors to save pvals, coef, and means from 100 iterations
...
# reading in metabolism scores and creating binary scores
...
#keeping only people with scores and who haven't moved/moved but didn't switch schools
surv2 <- surv2[surv2$ffFam %in% score$ffFam,]
surv2 <- surv2 %>% select(ffFam, p6j1, p6j5)
merged_score <- merge(score, surv2, by="ffFam")
merged_score <- merged_score %>% filter(p6j1 == 2 | p6j5 == 0)



# doing this 100 times for nearest neighbor binary
...
#control matching (this is from MatchIt_11-1.R)--see comments explaining steps on that code
...
#only keeping people who didn't move or who moved but didn't switch schools
...
# keeping only individuals with  slow metabolisms
slow_ffFam <- merged_score$ffFam[merged_score$binary == 1] #this line is changed to merged_score$binary_alc==1 for alc score
merged4 <- merged4[merged4$ffFam %in% slow_ffFam,]
...
write.csv(df_loop, "/home/HPA/Hannah/telomere_metabolism_slow_alc.csv")
write.csv(df_means, "/home/HPA/Hannah/telomere_metabolism_slow_means_alc.csv")
